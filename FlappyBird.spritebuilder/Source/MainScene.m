...
- (void)didLoadFromCCB {
    ...
    // SpriteBuilder, at the moment, does not have a ParallaxNode,
    // so we need to manually take each of the clouds and bushes
    // and switch them to be children of the CCParalaxNode we just created using code
...
- (void)addObstacle {
    ...
    NSLog(@"screenPosition: %@", NSStringFromCGPoint(screenPosition));
    CGPoint worldPosition = [physicsNode convertToNodeSpace:screenPosition];
    NSLog(@"worldPosition: %@", NSStringFromCGPoint(worldPosition));
...
- (void)update:(CCTime)delta
{
    ...
    for (CCNode *bush in _bushes) {
        ...
        // Get the screen position of hte bush
    ...
    for (CCNode *cloud in _clouds) {
        ...
        // If the left corner is one complete width off teh screen,
        // move it to the right
